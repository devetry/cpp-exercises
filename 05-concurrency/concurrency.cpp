...
int runThreads()
{
    ...
    for (auto& th: threads) // For loop syntax
    {
        th.join(); // main thread waits for child thread to finish
    }
    return accum; // How are we sure all threads have completed here?
}
...
TEST_CASE( "We end up with 4950" )
{
    ...
    REQUIRE( std::all_of(tests.begin(), tests.end(), [](int i){ std::cout << i << std::endl; return i == 4950; }) );
}
